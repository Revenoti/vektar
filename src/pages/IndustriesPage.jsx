              ...
              <Card key={index} className="glass-card hover-glow">
                <CardHeader className="pb-2 sm:pb-4">
                  <div className="flex items-start justify-between">
                    <div className="w-10 h-10 sm:w-14 sm:h-14 vektar-gradient rounded-xl flex items-center justify-center">
                      <industry.icon className="w-5 h-5 sm:w-7 sm:h-7 text-white" />
                    </div>
                    <Badge className="vektar-gradient text-white border-0 text-xs">
                      {industry.results}
                    </Badge>
                  </div>
                  <CardTitle className="text-lg sm:text-xl mt-3 sm:mt-4">{industry.title}</CardTitle>
                  <p className="text-xs sm:text-sm text-muted-foreground">{industry.description}</p>
                </CardHeader>
                <CardContent className="space-y-3 sm:space-y-4">
                  <div>
                    <h4 className="font-semibold text-xs sm:text-sm mb-1 sm:mb-2 text-red-500">Common Challenges</h4>
                    <ul className="space-y-1">
                      {industry.challenges.map((challenge, idx) => (
                        <li key={idx} className="text-xs sm:text-sm text-muted-foreground flex items-start">
                          <span className="w-1.5 h-1.5 bg-red-500 rounded-full mt-1.5 sm:mt-2 mr-2 flex-shrink-0"></span>
                          {challenge}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    <h4 className="font-semibold text-xs sm:text-sm mb-1 sm:mb-2 text-green-500">Our Solutions</h4>
                    <ul className="space-y-1">
